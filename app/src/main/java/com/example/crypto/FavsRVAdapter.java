...
public class FavsRVAdapter extends RecyclerView.Adapter<FavsRVAdapter.CurrencyViewholder> {
    private static DecimalFormat df2 = new DecimalFormat("#.##");
    private ArrayList<FavsModal> favsModals;
    private Context context;

    public FavsRVAdapter(ArrayList<FavsModal> favsModals, Context context) {
        this.favsModals = favsModals;
        this.context = context;
    }

    // below is the method to filter our list.
    public void filterList(ArrayList<FavsModal> filterllist) {
        // adding filtered list to our
        // array list and notifying data set changed
        favsModals = filterllist;
        notifyDataSetChanged();
    }

    @NonNull
    @Override
    public FavsRVAdapter.CurrencyViewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        // this method is use to inflate the layout file
        // which we have created for our recycler view.
        // on below line we are inflating our layout file.
        View view = LayoutInflater.from(context).inflate(R.layout.currency_rv_item, parent, false);
        return new FavsRVAdapter.CurrencyViewholder(view);
    }


    @Override
    public void onBindViewHolder(@NonNull FavsRVAdapter.CurrencyViewholder holder, int position) {
        // on below line we are setting data to our item of
        // recycler view and all its views.
        FavsModal modal = favsModals.get(position);
